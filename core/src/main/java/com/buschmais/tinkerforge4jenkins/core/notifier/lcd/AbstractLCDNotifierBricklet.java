...
public abstract class AbstractLCDNotifierBricklet<T extends Device, C extends AbstractLCDConfigurationType>
	...
	/**
	 * Return the maximum number of columns that can be dislayed on the
	 * bricklet.
	 * 
	...
	public void postUpdate() {
		...
		Set<JobState> successfulJobs = filter(
				getJobsByBuildState(BuildStateType.SUCCESS), filter);
	...
	private void writeJobStates(Set<JobState> jobs) {
		...
		while (iterator.hasNext() && i < getMaximumRows()) {
			...
			if (!BuildStateType.SUCCESS.equals(summary.getBuildState())) {
	 ...
	 * given job name and {@link BuildStateType}.
	 * 
	 * @param jobName
	 *            The job name.
	 * @param buildState
	 *            The {@link BuildStateType}.
	 * @return The message.
	 */
	public String createStatusLine(String jobName, BuildStateType buildState) {
	...
	/**
	 * Returns the symbol to display for the given {@link BuildStateType}.
	 * 
	 * @param buildState
	 *            The {@link BuildStateType}.
	 * @return The symbol or '?' if the state is unknown.
	 */
	private char getBuildStateSymbol(BuildStateType buildState) {
		switch (buildState) {
		case ABORTED:
			return 'A';
		case NOT_BUILT:
